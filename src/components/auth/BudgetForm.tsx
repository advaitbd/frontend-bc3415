...
export const BudgetForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const requestBody = {
        ...
        budget: customAmount ? parseInt(customAmount) : budgetRanges.find(budget => budget.id === selectedBudget)?.upperLimit || 0,
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/user_profile/user_profile/`, {
        method: 'POST',
        headers: {
  ...
  const getBudgetIcon = (id: string) => {
    switch (id) {
      case 'starter': return <PiggyBank className="w-5 h-5" />;
      case 'growth': return <Wallet className="w-5 h-5" />;
      case 'premium': return <Briefcase className="w-5 h-5" />;
      default: return null;
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 gap-4 p-4">
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600 mt-2">Select your investment budget range</p>
      </div>
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <div className="flex items-center justify-center w-6 h-6 border rounded">
              {selectedBudget === budget.id && <div className="w-4 h-4 bg-blue-600 rounded-sm" />}
            </div>
            {getBudgetIcon(budget.id)}
            <span className="font-medium">{budget.name}</span>
          </div>
          <div className="ml-9">
            <p className="text-lg font-semibold text-blue-600 mb-1">{budget.range}</p>
